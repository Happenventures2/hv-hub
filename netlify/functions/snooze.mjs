...
export default async (req) => {
  ...
  try {
    const { recordId, days } = await req.json();
    if (!recordId || !days) return json({ error: "Missing recordId or days" }, 400);
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({ fields: { "Due Date": isoDate } }),
    });
    ...
    return json({ ok: true, newDueDate: isoDate });
  } catch (err) {
...
export const config = { path: "/api/snooze" };
